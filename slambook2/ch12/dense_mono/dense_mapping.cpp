...
const int ncc_area = (2 * ncc_window_size + 1) * (2 * ncc_window_size + 1); // NCC窗口面积    7*7
...
int main(int argc, char **argv) {
    ...
    for (int index = 1; index < color_image_files.size()-1; index++) {
        ...
        SE3d pose_T_C_R = pose_curr_TWC.inverse() * pose_ref_TWC;   // 坐标转换关系： T_C_W * T_W_R = T_C_R    T12 = T2^-1 * T1
        //对整个深度图进行更新
...
bool readDatasetFiles(
    ...
    for (int y = 0; y < height; y++)
        for (int x = 0; x < width; x++) {
            double depth = 0;
            fin >> depth;
            ref_depth.ptr<double>(y)[x] = depth / 100.0;   //cm
        }

    return true;
}

// 对整个深度图进行更新  //第一张图、当前图、之间转换矩阵、深度图、深度协方差图
...
bool epipolarSearch(
    ...
    Vector3d f_ref = px2cam(pt_ref);  //u v  -- 归一化平面坐标
    f_ref.normalize();
...
bool updateDepthFilter(
    ...
    double depth_estimation = p_esti.norm();   // 观测到的深度值  
    ...
    double d_cov2 = d_cov * d_cov;   //观测值方差
